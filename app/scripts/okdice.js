...
(function(_, $, Backbone, Bacon) {
  ...
  function loadButtons(options) {
    ...
    function loadChatButtons(options) {
      ...
      var chatButtonTemplate = _.template('<button class="chat-btn" data-txt="<%= text %>"><%= label %></button>');
    ...
    function loadTableSelector(options) {
      ...
      var tableTemplate = _.template('<tr class="pts-<%= desc %>"><td class="table-name"><a href="#<%= name %>"><%= name %></a></td><td class="table-num-players"><%= playerCount %> <small>(<%= viewerCount %>)</small></td><td class="table-min-points"><%= desc.replace("000","k") %></td></tr>'),
